...
export default function AccountingPage() {
  ...
  const subModules = [
    { href: '/dashboard/accounting/sales', icon: Receipt, label: '銷項管理', desc: '發票開立記錄', color: 'bg-blue-50 text-blue-600' },
    { href: '/dashboard/accounting/expenses', icon: TrendingDown, label: '進項管理', desc: '各類支出記錄', color: 'bg-red-50 text-red-600' },
    { href: '/dashboard/accounting/payroll', icon: Users, label: '人事薪資', desc: '員工薪資與勞健保', color: 'bg-purple-50 text-purple-600' },
    { href: '/dashboard/accounting/projects', icon: BarChart3, label: '專案損益', desc: '各案件收支毛利', color: 'bg-green-50 text-green-600' },
    { href: '/dashboard/accounting/calculator', icon: Calculator, label: '利潤試算', desc: '報價毛利換算工具', color: 'bg-yellow-50 text-yellow-600' },
    { href: '/dashboard/accounting/reports', icon: FileText, label: '歷年報表', desc: '年度比較與趨勢', color: 'bg-indigo-50 text-indigo-600' },
  ]
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">年度收支總覽 · 僅限管理員</p>
          </div>
        </div>
        <select
          value={year}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-blue-50 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm text-blue-600 font-medium">年度銷售收入</span>
            </div>
            <p className="text-2xl font-bold text-blue-700">{fmt(totals.totalSales)}</p>
            <p className="text-xs text-blue-500 mt-1">{totals.salesCount} 筆銷項</p>
          </div>
          <div className="bg-red-50 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-1">
              <TrendingDown className="w-5 h-5 text-red-600" />
              <span className="text-sm text-red-600 font-medium">年度支出成本</span>
            </div>
            <p className="text-2xl font-bold text-red-700">{fmt(totals.totalExpenses)}</p>
            <p className="text-xs text-red-500 mt-1">{totals.expensesCount} 筆進項</p>
          </div>
          <div className="bg-purple-50 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-1">
              <Users className="w-5 h-5 text-purple-600" />
              <span className="text-sm text-purple-600 font-medium">人事費用</span>
            </div>
            <p className="text-2xl font-bold text-purple-700">{fmt(totals.totalPayroll)}</p>
            <p className="text-xs text-purple-500 mt-1">{totals.payrollCount} 筆薪資</p>
          </div>
          <div className={`rounded-xl p-5 ${totals.totalProfit >= 0 ? 'bg-green-50' : 'bg-orange-50'}`}>
            <div className="flex items-center gap-2 mb-1">
              <DollarSign className={`w-5 h-5 ${totals.totalProfit >= 0 ? 'text-green-600' : 'text-orange-600'}`} />
              <span className={`text-sm font-medium ${totals.totalProfit >= 0 ? 'text-green-600' : 'text-orange-600'}`}>年度利潤</span>
            </div>
            <p className={`text-2xl font-bold ${totals.totalProfit >= 0 ? 'text-green-700' : 'text-orange-700'}`}>{fmt(totals.totalProfit)}</p>
            <p className={`text-xs mt-1 ${totals.totalProfit >= 0 ? 'text-green-500' : 'text-orange-500'}`}>
      ...
      {!loading && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            <h2 className="font-semibold text-gray-800">{year} 年每月收支摘要</h2>
          </div>
              ...
              <tbody>
                {monthlySummary.map((row) => (
                  <tr key={row.month} className="border-t border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right text-blue-600">{row.sales > 0 ? fmt(row.sales) : '-'}</td>
                    <td className="px-4 py-3 text-right text-red-600">{row.expenses > 0 ? fmt(row.expenses) : '-'}</td>
                    <td className="px-4 py-3 text-right text-purple-600">{row.payroll > 0 ? fmt(row.payroll) : '-'}</td>
                    <td className={`px-4 py-3 text-right font-medium ${row.profit >= 0 ? 'text-green-600' : 'text-orange-600'}`}>
      ...
      {/* 子模組入口 */}
      <div>
        <h2 className="font-semibold text-gray-800 mb-3">功能模組</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {subModules.map(({ href, icon: Icon, label, desc, color }) => (
            <Link
              key={href}
              href={href}
              className="flex items-center gap-4 bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md hover:border-gray-300 transition-all group"
            >
