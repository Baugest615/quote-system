...
import PagePermissionGrid from '@/components/settings/PagePermissionGrid'
import {
  Shield,
  Pencil,
  Trash2,
...
type PermTabKey = 'users' | 'pages'

const PERM_TABS: { key: PermTabKey; label: string; icon: React.ReactNode }[] = [
  { key: 'users', label: '使用者管理', icon: <Users className="h-4 w-4" /> },
  { key: 'pages', label: '頁面權限', icon: <Lock className="h-4 w-4" /> },
]
...
export default function PermissionsPage() {
  ...
  const [activeTab, setActiveTab] = useState<PermTabKey>('users')
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">僅管理員可管理權限設定</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tab 內容：頁面權限 */}
      {activeTab === 'pages' && <PagePermissionGrid />}
    </div>
  )
}
