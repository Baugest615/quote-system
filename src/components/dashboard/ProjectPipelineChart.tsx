...
const PIPELINE_STAGES = [
  { key: '洽談中' as const, color: '#f59e0b', bgClass: 'bg-amber-500/15', textClass: 'text-amber-400' },
  { key: '執行中' as const, color: '#10b981', bgClass: 'bg-emerald-500/15', textClass: 'text-emerald-400' },
  { key: '結案中' as const, color: '#0ea5e9', bgClass: 'bg-sky-500/15', textClass: 'text-sky-400' },
  { key: '關案' as const, color: '#64748b', bgClass: 'bg-slate-500/15', textClass: 'text-slate-400' },
]
    ...
    <div className="bg-card border border-border rounded-xl p-5 sm:p-6">
      <div className="flex items-baseline justify-between mb-4">
        ...
        <span className="text-xs text-muted-foreground">
          共 <span className="font-bold text-foreground tabular-nums">{total}</span> 個專案
        </span>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {PIPELINE_STAGES.map((stage) => {
          ...
          return (
            ...
            >
              <p className={cn("text-lg font-bold tabular-nums", stage.textClass)}>
                {count}
              </p>
