  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      <div className="bg-card p-6 rounded-lg shadow">
        <h2 className="text-lg font-semibold text-foreground mb-4 flex items-center"><FileSignature className="mr-2 h-5 w-5 text-emerald-400" />基本資訊</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div><label className="block text-sm font-medium text-foreground/70 mb-1">專案名稱 *</label><Input {...register('project_name')} placeholder="請輸入專案名稱" />{errors.project_name && <p className="text-red-500 text-sm mt-1">{errors.project_name.message}</p>}</div>
          <div><label htmlFor="client-select" className="block text-sm font-medium text-foreground/70 mb-1">選擇客戶</label><Controller control={control} name="client_id" render={({ field: { onChange, value } }) => (<select id="client-select" value={value || ''} onChange={onChange} className="form-input" aria-label="選擇客戶"><option value="">-- 選擇客戶 --</option>{clients.map(client => (<option key={client.id} value={client.id}>{client.name}</option>))}</select>)} /></div>
          ...
          <div><label className="block text-sm font-medium text-foreground/70 mb-1">電子郵件</label><Input value={clientInfo.email} readOnly className="bg-secondary/50" placeholder="選擇客戶後自動填入" /></div>
          <div><label className="block text-sm font-medium text-foreground/70 mb-1">統一編號</label><Input value={clientInfo.tin} readOnly className="bg-secondary/50" placeholder="選擇客戶後自動填入" /></div>
          <div><label className="block text-sm font-medium text-foreground/70 mb-1">發票抬頭</label><Input value={clientInfo.invoiceTitle} readOnly className="bg-secondary/50" placeholder="選擇客戶後自動填入" /></div>
          ...
          {/* 其他欄位維持不變 */}
          <div><label htmlFor="status-select" className="block text-sm font-medium text-foreground/70 mb-1">狀態</label><Controller control={control} name="status" render={({ field: { value } }) => (<div className="space-y-2"><select id="status-select" value={value || '草稿'} onChange={(e) => handleStatusChange(e.target.value as QuotationStatus)} className="form-input w-full"><option value="草稿">草稿</option><option value="待簽約">待簽約</option><option value="已簽約">已簽約</option><option value="已歸檔">已歸檔</option></select>{!hasAttachment(initialData?.attachments) && value !== '草稿' && (<p className="text-xs text-amber-600 flex items-center"><svg className="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" /></svg>需上傳雙方用印的委刊報價單才能設為「已簽約」</p>)}</div>)} /></div>
          <div><label className="block text-sm font-medium text-foreground/70 mb-1">地址</label><Input value={clientInfo.address} readOnly className="bg-secondary/50" placeholder="選擇客戶後自動填入" /></div>
        </div>
        <div className="mt-6"><label className="block text-sm font-medium text-foreground/70 mb-2">付款方式</label><div className="flex space-x-4"><label className="flex items-center"><input type="radio" {...register('payment_method')} value="電匯" className="form-radio" /><span className="ml-2 text-sm">電匯</span></label><label className="flex items-center"><input type="radio" {...register('payment_method')} value="ATM轉帳" className="form-radio" /><span className="ml-2 text-sm">ATM轉帳</span></label></div></div>
      </div>

      {/* --- 報價項目表格 --- */}
      <div className="bg-card p-6 rounded-lg shadow">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold text-foreground flex items-center"><Calculator className="mr-2 h-5 w-5 text-emerald-400" />報價項目</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-secondary">
                <th className="p-2 w-[120px] text-left font-medium text-muted-foreground">類別</th>
                <th className="p-2 w-[200px] text-left font-medium text-muted-foreground">名稱/項目</th>
                <th className="p-2 w-[220px] text-left font-medium text-muted-foreground">執行內容</th>
                <th className="p-2 w-[120px] text-left font-medium text-muted-foreground">單價</th>
                <th className="p-2 w-[120px] text-left font-medium text-muted-foreground">成本</th>
                <th className="p-2 w-[80px] text-left font-medium text-muted-foreground">數量</th>
                <th className="p-2 w-[120px] text-left font-medium text-muted-foreground">合計</th>
                <th className="p-2 w-[80px] text-center font-medium text-muted-foreground">操作</th>
              </tr>
            </thead>
            <tbody>
              {fields.map((field, index) => {
                ...
                return (
                  <tr key={field.id} className="align-top border-b table-row-min-height">
                    ...
                    <td className="p-3 align-top"><div className="text-sm font-semibold text-foreground/70 py-2">NT$ {itemTotal.toLocaleString()}</div></td>
      ...
      {/* --- 金額計算 & 合約條款 (維持不變) --- */}
      <div className="bg-card p-6 rounded-lg shadow"><h2 className="text-lg font-semibold text-foreground mb-4 flex items-center"><Calculator className="mr-2 h-5 w-5 text-emerald-400" />金額計算</h2><div className="space-y-3"><div className="flex justify-between text-sm"><span>小計（未稅）:</span><span>NT$ {subTotalUntaxed.toLocaleString()}</span></div><div className="flex justify-between text-sm"><span>稅金 (5%):</span><span>NT$ {tax.toLocaleString()}</span></div><div className="flex justify-between font-semibold text-lg border-t pt-2"><span>合計（含稅）:</span><span>NT$ {grandTotalTaxed.toLocaleString()}</span></div><div className="mt-4"><label className="flex items-center space-x-2"><input type="checkbox" {...register('has_discount')} className="form-checkbox" /><span className="text-sm font-medium">是否有未稅優惠價格</span></label>{watchHasDiscount && (<div className="mt-2"><Input type="number" {...register('discounted_price', { valueAsNumber: true })} placeholder="優惠後價格" className="w-48" /></div>)}</div></div></div>
      <div className="bg-card p-6 rounded-lg shadow"><h2 className="text-lg font-semibold text-foreground mb-4 flex items-center"><Book className="mr-2 h-5 w-5 text-emerald-400" />合約條款與備註</h2><div className="space-y-4"><div><label className="block text-sm font-medium text-foreground/70 mb-1">合約條款</label><Textarea {...register('terms')} rows={10} placeholder="合約條款內容" /></div><div><label className="block text-sm font-medium text-foreground/70 mb-1">備註</label><Textarea {...register('remarks')} rows={3} placeholder="其他備註事項" /></div></div></div>
