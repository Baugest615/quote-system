// src/components/quotes/QuoteForm.tsx - 最終功能完整版
'use client'
...
import { z } from 'zod'
import { toast } from 'sonner' // 步驟 1：引入 toast

// --- Type Definitions (維持不變) ---
...
interface FormItem { id?: string; quotation_id?: string | null; category?: string | null; kol_id?: string | null; service: string; quantity: number; price: number; remark?: string | null; created_at?: string | null; }
const quoteSchema = z.object({ project_name: z.string().min(1, '專案名稱為必填項目'), client_id: z.string().nullable(), client_contact: z.string().nullable(), payment_method: z.enum(['電匯', 'ATM轉帳']), status: z.enum(['草稿', '待簽約', '已簽約', '已歸檔']).optional(), has_discount: z.boolean(), discounted_price: z.number().nullable(), terms: z.string().nullable(), remarks: z.string().nullable(), items: z.array(z.object({ id: z.string().optional(), quotation_id: z.string().nullable().optional(), category: z.string().nullable().optional(), kol_id: z.string().nullable().optional(), service: z.string().min(1, '執行內容為必填'), quantity: z.number().min(1, '數量必須大於0'), price: z.number().min(0, '價格不能為負數'), remark: z.string().nullable().optional(), created_at: z.string().nullable().optional(), })).min(1, "請至少新增一個報價項目"), });
type QuoteFormData = z.infer<typeof quoteSchema>;
interface QuoteFormProps { initialData?: Quotation & { quotation_items: QuotationItem[] } }

// --- 【全新】Portal Dropdown 元件，負責定位和渲染 ---
const PortalDropdown = ({ isOpen, children, triggerRef, className = '' }: { isOpen: boolean; children: React.ReactNode; triggerRef: React.RefObject<HTMLElement>; className?: string }) => {
...
// --- 【重構】各個搜尋元件，保留完整邏輯，但由外部控制開關 ---
const CategorySearchInput = ({ value, onChange, isOpen, onOpen, placeholder, categories }: { value: string | null, onChange: (v: string) => void, isOpen: boolean, onOpen: () => void, placeholder: string, categories: QuoteCategory[] }) => {
        ...
        <div ref={triggerRef}>
            <Input
                ...
                onChange={(e) => {
                    ...
                    onChange(e.target.value); // 允許手動輸入
                }}
                onFocus={onOpen}
            ...
            <PortalDropdown isOpen={isOpen} triggerRef={triggerRef}>
                {filteredCategories.map(cat => (
                    <div key={cat.id} onClick={() => handleSelect(cat.name)} className="px-3 py-2 text-sm cursor-pointer hover:bg-gray-100">{cat.name}</div>
                ))}
            </PortalDropdown>
        </div>
    );
...
const KolSearchInput = ({ value, onChange, isOpen, onOpen, placeholder, kols }: { value: string | null, onChange: (v: string) => void, isOpen: boolean, onOpen: () => void, placeholder: string, kols: KolWithServices[] }) => {
    ...
    const filteredKols = searchTerm.trim()
        ? kols.filter(kol => kol.name.toLowerCase().includes(searchTerm.toLowerCase()) || (kol.real_name && kol.real_name.toLowerCase().includes(searchTerm.toLowerCase())))
        : kols;
        
    const handleSelect = (kol: KolWithServices) => {
        onChange(kol.id);
        ...
        <div ref={triggerRef}>
            <div className="relative">
                ...
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center space-x-1">
                    {value && <button type="button" onClick={handleClear} className="text-gray-400 hover:text-gray-600"><X className="h-4 w-4" /></button>}
                </div>
            </div>
            <PortalDropdown isOpen={isOpen} triggerRef={triggerRef} className="dropdown-wide dropdown-scrollable">
                {filteredKols.map((kol) => (
                    <div key={kol.id} onClick={() => handleSelect(kol)} className="w-full px-4 py-3 text-left hover:bg-blue-50 cursor-pointer">
                        <div className="flex flex-col">
                            <span className="font-medium text-sm text-gray-900">{kol.name}</span>
                            <span className="text-xs text-blue-600 mt-1">{kol.kol_services.length} 個服務項目</span>
                        </div>
                    </div>
                ))}
            </PortalDropdown>
...
const ServiceSearchInput = ({ value, onChange, isOpen, onOpen, placeholder, kolServices }: { value: string, onChange: (service: string, price?: number) => void, isOpen: boolean, onOpen: () => void, placeholder: string, kolServices: (KolService & { service_types: ServiceType })[] }) => {
    const [searchTerm, setSearchTerm] = useState(value || '');
    const triggerRef = useRef<HTMLDivElement>(null);
    useEffect(() => { setSearchTerm(value || '') }, [value]);
    ...
    const handleSelect = (service: (KolService & { service_types: ServiceType })) => {
        onChange(service.service_types.name, service.price);
        setActiveDropdown(null);
    };
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSearchTerm(term);
        onChange(term, undefined); // 手動輸入時不帶入價格
    }
    ...
    return (
        <div ref={triggerRef}>
            <Input
                ...
                placeholder={placeholder}
            />
            <PortalDropdown isOpen={isOpen} triggerRef={triggerRef} className="dropdown-medium dropdown-scrollable">
                {filteredServices.length > 0 ? (
                    filteredServices.map(service => (
                        <div key={service.id} onClick={() => handleSelect(service)} className="px-3 py-2 text-sm cursor-pointer hover:bg-gray-100">{service.service_types.name}</div>
                    ))
                ) : (
...
const setActiveDropdown = (id: string | null) => {
    activeDropdown = id;
    // This is a bit of a hack to force re-render, a proper state management solution (like Zustand or Context) would be better.
    // For now, this will trigger the necessary updates.
    document.dispatchEvent(new Event('activeDropdownChange'));
};


const transformInitialItems = (items?: QuotationItem[]): FormItem[] => { if (!items || items.length === 0) { return [{ category: null, kol_id: null, service: '', quantity: 1, price: 0, remark: null }] } return items.map((item): FormItem => ({ id: item.id, quotation_id: item.quotation_id, category: item.category, kol_id: item.kol_id, service: item.service, quantity: item.quantity || 1, price: item.price, remark: item.remark, created_at: item.created_at, })) }
...
export default function QuoteForm({ initialData }: QuoteFormProps) {
  ...
  const [, forceUpdate] = useState({}); // Force re-render for activeDropdown
  const formRef = useRef<HTMLFormElement>(null);

  const {
    register, handleSubmit, control, watch, setValue,
    formState: { errors, isSubmitting },
  } = useForm<QuoteFormData>({ resolver: zodResolver(quoteSchema), defaultValues: { project_name: initialData?.project_name || '', client_id: initialData?.client_id || null, client_contact: initialData?.client_contact || null, payment_method: initialData?.payment_method || '電匯', status: initialData?.status || '草稿', has_discount: initialData?.has_discount || false, discounted_price: initialData?.discounted_price || null, terms: initialData?.terms || staticTerms.standard, remarks: initialData?.remarks || '', items: transformInitialItems(initialData?.quotation_items), }, });
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
        // 如果點擊的不是輸入框或下拉選單本身，就關閉
        if (!(event.target as HTMLElement).closest('.relative > .relative, .dropdown-fixed')) {
            setActiveDropdown(null);
        }
    };
    ...
    return () => {
        document.removeEventListener('activeDropdownChange', handleDropdownChange);
  ...
  useEffect(() => {
    async function fetchData() {
        const [clientsRes, kolsRes, categoriesRes] = await Promise.all([ supabase.from('clients').select('*').order('name'), supabase.from('kols').select('*, kol_services(*, service_types(*))').order('name'), supabase.from('quote_categories').select('*').order('name'), ]);
        setClients(clientsRes.data || []); setKols((kolsRes.data as KolWithServices[]) || []); setQuoteCategories(categoriesRes.data || []); setLoading(false);
    }
    fetchData();
  }, []);
  ...
  useEffect(() => {
    ...
    if (selectedClient) { setValue('client_contact', selectedClient.contact_person); setClientInfo({ tin: selectedClient.tin || '', invoiceTitle: selectedClient.invoice_title || '', address: selectedClient.address || '', email: selectedClient.email || '' });
    } else { setValue('client_contact', ''); setClientInfo({ tin: '', invoiceTitle: '', address: '', email: '' }); }
  }, [watchClientId, clients, setValue]);

  const handleKolChange = (itemIndex: number, kolId: string) => { setValue(`items.${itemIndex}.kol_id`, kolId || null); setValue(`items.${itemIndex}.service`, ''); setValue(`items.${itemIndex}.price`, 0); };
  ...
  const onSubmit: SubmitHandler<QuoteFormData> = async (data) => {
    ...
    try {
        ...
        if (quoteId) { const { error } = await supabase.from('quotations').update(quoteDataToSave).eq('id', quoteId); if (error) throw error;
        ...
        const itemsToInsert = data.items.filter(item => item.service || item.price).map(item => ({ quotation_id: quoteId, category: item.category || null, kol_id: item.kol_id || null, service: item.service || '', quantity: Number(item.quantity) || 1, price: Number(item.price) || 0, remark: item.remark || null }));
        if (itemsToInsert.length > 0) { const { error } = await supabase.from('quotation_items').insert(itemsToInsert); if (error) throw error; }
        
        toast.success('儲存成功！'); // 使用 toast 顯示成功訊息
        
        router.push('/dashboard/quotes'); 
        router.refresh();
    } catch (error: any) { 
        console.error('Save failed:', error); 
        toast.error('儲存失敗: ' + error.message); // 使用 toast 顯示錯誤訊息
    }
  };
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit(onSubmit)} className="space-y-8">
       <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">聯絡人</label><Input {...register('client_contact')} placeholder="聯絡人姓名" /></div>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <div className="flex justify-between items-center mb-4">
            ...
            <Button type="button" onClick={() => append({ category: null, kol_id: null, service: '', quantity: 1, price: 0, remark: null })}><PlusCircle className="mr-2 h-4 w-4" /> 新增項目</Button>
        </div>
                       ...
                       return (
                        <tr key={field.id} className="align-top border-b table-row-min-height">
                           <td className="p-3 align-top" onMouseDown={e => e.stopPropagation()}>
                                <CategorySearchInput
                                    value={watchItems[index]?.category || ''}
                                    onChange={(value) => setValue(`items.${index}.category`, value)}
                                    categories={quoteCategories}
                                    isOpen={activeDropdown === categoryId}
                                    onOpen={() => setActiveDropdown(categoryId)}
                                    placeholder="類別"
                                />
                            </td>
                            <td className="p-3 align-top" onMouseDown={e => e.stopPropagation()}>
                               <KolSearchInput
                                    ...
                                    isOpen={activeDropdown === kolId}
                                    onOpen={() => setActiveDropdown(kolId)}
                                    placeholder="搜尋 KOL"
                                    kols={kols}
                               />
                            </td>
                           <td className="p-3 align-top" onMouseDown={e => e.stopPropagation()}>
                                <ServiceSearchInput
                                    value={watchItems[index]?.service}
                                    onChange={(service, price) => { setValue(`items.${index}.service`, service); if (price !== undefined) setValue(`items.${index}.price`, price); }}
                                    isOpen={activeDropdown === serviceId}
                                    onOpen={() => setActiveDropdown(serviceId)}
                                    placeholder="搜尋或輸入服務"
                            ...
                            <td className="p-3 text-center align-top"><Button type="button" variant="ghost" size="sm" onClick={() => remove(index)}><Trash2 className="h-4 w-4 text-red-500" /></Button></td>
                        </tr>
                       )
                    })}
                </tbody>
       ...
       <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="mt-4"><label className="flex items-center space-x-2"><input type="checkbox" {...register('has_discount')} className="form-checkbox" /><span className="text-sm font-medium">是否有優惠價格</span></label>{watchHasDiscount && (<div className="mt-2"><Input type="number" {...register('discounted_price', { valueAsNumber: true })} placeholder="優惠後價格" className="w-48" /></div>)}</div>
            </div>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-lg font-semibold text-gray-800 mb-4 flex items-center"><Book className="mr-2 h-5 w-5 text-indigo-500" />合約條款與備註</h2>
            <div className="space-y-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">合約條款</label><Textarea {...register('terms')} rows={10} placeholder="合約條款內容" /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">執行時間</label><Textarea {...register('remarks')} rows={3} placeholder="其他備註事項" /></div>
            </div>
        </div>
