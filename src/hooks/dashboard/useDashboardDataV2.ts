...
export interface ActivityTimelineItem {
  id: string
  type: 'project_created' | 'project_status_change' | 'quote_signed' | 'quote_created'
  title: string
  subtitle: string
  timestamp: string
  status?: string
...
function getMonthIndex(dateStr: string, buckets: { start: Date }[]): number {
  ...
  return buckets.findIndex(
    (b) => b.start.getFullYear() === d.getFullYear() && b.start.getMonth() === d.getMonth()
  )
}
...
async function fetchDashboardDataV2(): Promise<DashboardDataV2> {
  ...
  const [projectsRes, quotationsRes, pendingProjectReviewRes, pendingExpenseReviewRes] =
    await Promise.all([
      supabase
        .from('projects')
  ...
  const signedThisMonth = quotations.filter(
    (q) => q.status === '已簽約' && q.updated_at && q.updated_at >= thisMonthStart
  ).length

  const pendingSignature = quotations.filter((q) => q.status === '待簽約').length
  const pendingActions = pendingSignature + pendingProjectReview + pendingExpenseReview
  ...
  for (const p of projects) {
    ...
    if (idx >= 0) {
      ...
      if (p.status === '執行中' || p.status === '洽談中' || p.status === '結案中') {
  ...
  const quoteStatusDistribution = Object.entries(statusCounts).map(([name, value]) => ({
    name,
    value,
