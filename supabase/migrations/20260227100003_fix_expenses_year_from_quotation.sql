-- =====================================================
-- 修正 accounting_expenses (KOL請款) 年月邏輯
-- 改用報價單建立日期的年月，與 accounting_sales 一致
-- 讓專案損益頁面收支在同一年度顯示
-- 注意：個人報帳(expense_claims)不受影響，維持原邏輯
-- =====================================================

-- ============================================================
-- 1. 修改 approve_payment_request RPC
--    年月改用 quotation.created_at
-- ============================================================
...
-- ============================================================
-- 2. 回填：用報價單 created_at 更新 KOL 請款產生的 expenses 年月
--    個人報帳(expense_claim_id)不受影響
-- ============================================================

UPDATE accounting_expenses AS ae
SET
  year = EXTRACT(YEAR FROM q.created_at)::integer,
  expense_month = TO_CHAR(q.created_at, 'YYYY') || '年' || EXTRACT(MONTH FROM q.created_at)::integer || '月'
FROM payment_requests pr
JOIN quotation_items qi ON qi.id = pr.quotation_item_id
JOIN quotations q ON q.id = qi.quotation_id
WHERE ae.payment_request_id = pr.id
  AND ae.payment_request_id IS NOT NULL;
