...
-- (removed: pg_net extension managed by Supabase)

CREATE TRIGGER on_auth_user_created AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- (removed: storage schema triggers managed by Supabase)


  create policy "attachments 1mt4rzk_0"
  on "storage"."objects"
  as permissive
  for select
  to public
using ((bucket_id = 'attachments'::text));
  ...
  create policy "attachments 1mt4rzk_1"
  on "storage"."objects"
  as permissive
  for insert
  to public
with check ((bucket_id = 'attachments'::text));
  ...
  create policy "attachments 1mt4rzk_2"
  on "storage"."objects"
  as permissive
  for update
  to public
using ((bucket_id = 'attachments'::text));
  ...
  create policy "attachments 1mt4rzk_3"
  on "storage"."objects"
  as permissive
  for delete
  to public
...
-- (removed: storage protect triggers managed by Supabase)
