-- ============================================================
-- Migration: 修復被駁回項目在工作台消失的 Bug
--
-- 問題：get_workbench_items() 用 q.status IN ('已簽約', '已歸檔') 過濾，
--       但使用者可以自由變更報價單狀態。當項目被駁回後、報價單狀態被改回
--       '草稿'/'待簽約'，被駁回的項目就從工作台消失了。
--
-- 修復：擴展 WHERE 條件，讓 rejected_at IS NOT NULL 的項目也出現。
--       這些項目曾走過送出→駁回流程，不論報價單當前狀態，都應顯示。
-- ============================================================
...
IS '請款工作台：取得所有需處理的項目（v1.1 hotfix: 被駁回的項目不受報價單狀態限制）';
