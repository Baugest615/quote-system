-- ============================================================
-- Migration: cost_amount 不再乘以 quantity
--
-- 修正：cost_amount 應直接等於 cost（成本單價），不需乘以 quantity。
--       之前的 migration 20260303190000 錯誤地以 cost * quantity 回補。
--
-- 變更：
--   1. 回補修正：quantity > 1 且 cost_amount = cost * quantity 的項目
--   2. get_workbench_items RPC COALESCE fallback 改為 cost（不乘 quantity）
-- ============================================================

-- 1. 回補修正：將被錯誤乘算的 cost_amount 還原為 cost
UPDATE public.quotation_items
SET cost_amount = cost
WHERE quantity > 1
  AND cost IS NOT NULL
  AND cost > 0
  AND cost_amount = cost * quantity;

-- 2. get_workbench_items：COALESCE fallback 改為 cost（不乘 quantity）
...
BEGIN
  ...
  SELECT
    ...
    qi.price,
    qi.cost,
    COALESCE(NULLIF(qi.cost_amount, 0), qi.cost, 0::numeric) AS cost_amount,
    qi.invoice_number,
    qi.attachments,
    qi.expense_type,
...
IS '請款工作台：取得所有需處理的項目（cost_amount fallback 為 cost，不乘 quantity）';
